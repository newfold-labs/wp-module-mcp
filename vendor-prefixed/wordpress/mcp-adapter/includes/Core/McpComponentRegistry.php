...
use Bluehost\Plugin\WP\MCP\Domain\Prompts\RegisterAbilityAsMcpPrompt;
use Bluehost\Plugin\WP\MCP\Domain\Resources\McpResource;
use Bluehost\Plugin\WP\MCP\Domain\Resources\RegisterAbilityAsMcpResource;
use Bluehost\Plugin\WP\MCP\Domain\Tools\McpTool;
use Bluehost\Plugin\WP\MCP\Domain\Tools\RegisterAbilityAsMcpTool;
...
class McpComponentRegistry {
	 ...
	 * Constructor.
	 *
	 * @param \Bluehost\Plugin\WP\MCP\Core\McpServer $mcp_server MCP server instance.
	 * @param \Bluehost\Plugin\WP\MCP\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface $error_handler Error handler instance.
	 * @param \Bluehost\Plugin\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface $observability_handler Observability handler instance.
	...
	public function register_tools( array $tools ): void {
		foreach ( $tools as $tool_item ) {
			...
			$ability = blu_wp_get_ability( $tool_item );
			...
			if ( $this->mcp_validation_enabled ) {
				...
				$validation_result = \Bluehost\Plugin\WP\MCP\Domain\Tools\McpToolValidator::validate_tool_instance( $tool, $context_to_use );
	 ...
	 *
	 * @param \Bluehost\Plugin\WP\MCP\Domain\Tools\McpTool $tool The tool instance to register.
	 *
	 * @return void
	 */
	...
	public function register_resources( array $abilities ): void {
		foreach ( $abilities as $ability_name ) {
			...
			$ability = blu_wp_get_ability( $ability_name );
	...
	public function register_prompts( array $prompts ): void {
		foreach ( $prompts as $prompt_item ) {
			...
			if ( class_exists( $prompt_item ) && in_array( McpPromptBuilderInterface::class, class_implements( $prompt_item ) ?: array(), true ) ) {
				...
				try {
					/** @var \Bluehost\Plugin\WP\MCP\Domain\Prompts\Contracts\McpPromptBuilderInterface $builder */
				...
				if ( $this->mcp_validation_enabled ) {
					...
					$validation_result = \Bluehost\Plugin\WP\MCP\Domain\Prompts\McpPromptValidator::validate_prompt_instance( $prompt, $context_to_use );
				...
				$ability = blu_wp_get_ability( $prompt_item );
	 ...
	 *
	 * @return \Bluehost\Plugin\WP\MCP\Domain\Tools\McpTool[]
	 */
	 ...
	 *
	 * @return \Bluehost\Plugin\WP\MCP\Domain\Prompts\McpPrompt[]
	 */

...
use function Bluehost\Plugin\WP_CLI\Utils\format_items;
 ...
 */
class McpCommand extends \BLU_WP_CLI_Command { // phpcs:ignore
	...
	public function serve( array $args, array $assoc_args ): void {
		...
		if ( empty( $servers ) ) {
			\BLU_WP_CLI::error( 'No MCP servers are registered. Please register at least one server first.' );
		}
		...
		if ( $server_id ) {
			...
			if ( ! $server ) {
				\BLU_WP_CLI::error( sprintf( 'Server with ID "%s" not found.', $server_id ) );
			}
		} else {
			// Use the first available server
			$server    = array_values( $servers )[0];
			$server_id = $server->get_server_id();
			\BLU_WP_CLI::line( sprintf( 'Using server: %s', $server_id ) );
		}
		...
		if ( isset( $assoc_args['user'] ) ) {
			...
			if ( ! $user ) {
				\BLU_WP_CLI::error( sprintf( 'User "%s" not found.', $assoc_args['user'] ) );
			}

			wp_set_current_user( $user->ID );
			\BLU_WP_CLI::debug( sprintf( 'Running as user: %s (ID: %d)', $user->user_login, $user->ID ) );
		} else {
			\BLU_WP_CLI::debug( 'Running without authentication. Some capabilities may be limited.' );
		}

		// Create and start STDIO server bridge
		try {
			\BLU_WP_CLI::debug( sprintf( 'Starting STDIO bridge for server: %s', $server_id ) );
			...
			\BLU_WP_CLI::error( $e->getMessage() );
		} catch ( \Throwable $e ) {
			\BLU_WP_CLI::error( 'Failed to start STDIO bridge: ' . $e->getMessage() );
		}
	}
	...
	public function list( array $args, array $assoc_args ): void {
		...
		if ( empty( $servers ) ) {
			\BLU_WP_CLI::line( 'No MCP servers registered.' );
			return;
		}

...
if ( ! class_exists( 'BLU_WP_REST_Abilities_Init' ) ) {
	require_once __DIR__ . '/rest-api/class-wp-rest-abilities-init.php';

	// Initialize REST API routes when WordPress is available.
	if ( function_exists( 'add_action' ) ) {
		add_action( 'rest_api_init', array( 'BLU_WP_REST_Abilities_Init', 'register_routes' ), 11 );
	}
}

// Load assets init class for plugin bootstrap.
if ( ! class_exists( 'BLU_WP_Abilities_Assets_Init' ) ) {
	require_once __DIR__ . '/assets/class-wp-abilities-assets-init.php';

	// Initialize client assets when WordPress is available.
	if ( function_exists( 'add_action' ) ) {
		add_action( 'init', array( 'BLU_WP_Abilities_Assets_Init', 'register_assets' ) );
		add_action( 'admin_enqueue_scripts', array( 'BLU_WP_Abilities_Assets_Init', 'admin_enqueue_scripts' ) );
	}
}
